...
"""
Autonomous navigation of swarm of robots within an unknown environment:
...
def move_obstacles(obstacles, params):
    ...
    obstacles[-3] += np.array([0.015, 0.0]) * params.robot_vel
    obstacles[-2] += np.array([-0.005, 0.005]) * params.robot_vel / 2
    obstacles[-1] += np.array([0.0, 0.008]) * params.robot_vel / 2
    return obstacles
...
class Params:
    def __init__(self):
        ...
        self.max_sp_dist = 0.2 * self.robot_vel  # * np.sqrt(self.num_robots)
        # [m], maximum distance between current robot's pose and
        # the sp from global planner
...
class Robot:
    ...
    def local_planner(self, obstacles, params):
        ...
        self.V = params.robot_vel * np.array([ax, ay])
        self.vel_array.append(norm(self.V))
        dt = 0.01 * params.robot_vel / norm([ax, ay]) if norm([ax, ay]) != 0 else 0.01
...
def visualize2D():
    ...
    robots_positions = []
    for robot in robots:
        plt.plot(robot.sp[0], robot.sp[1], '^', color='blue', markersize=10, zorder=15)  # robots positions
        robots_positions.append(robot.sp)
    robots_positions.sort(key=lambda p: atan2(p[1] - centroid[1], p[0] - centroid[0]))
    plt.gca().add_patch(Polygon(robots_positions, color='yellow'))
    plt.plot(centroid[0], centroid[1], '*', color='b', markersize=10, label='Centroid position')
    for robot in robots[1:]:
        plt.plot(robot.route[:, 0], robot.route[:, 1], '--', linewidth=2, color='green', zorder=10)
...
if __name__ == '__main__':
    ...
    robots_sp = formation(params.num_robots, leader_des=[1.2, 1.], v=np.array([0, -1]), l=params.interrobots_dist)
    robots[0].sp = np.array([1.2, 1.])
    robots[0].route = np.array([[1.2, 1.]])
    for i in range(len(robots_sp)):
        robots[i + 1].sp = robots_sp[i]
        robots[i + 1].route = np.array([robots_sp[i]])
    ...
    while True:  # loop through all the setpoint from global planner trajectory, traj_global
        ...
        if dist_to_goal < params.goal_tolerance:  # [m]
            ...
            break
        # change positions of some obstacles on the map
        if len(obstacles) > 2:
            obstacles = move_obstacles(obstacles, params)
        ...
        # leader's set point from global planner
        ...
        # formation positions from global planner
        ...
        """ adding robots in the swarm """
        # formation positions from global planner
        for p in range(len(robots_sp)):  # formation positions correction with local planner
            # robots repel from each other inside the formation
            robots_obstacles_sp = [x for i, x in enumerate(robots_sp + [robot1.sp]) if
                                   i != p]  # all positions except the robot[p]
            robots_obstacles = poses2polygons(
                robots_obstacles_sp)  # each robot is defined as a small cube for inter-robots collision avoidance
            obstacles1 = np.array(
                obstacles + robots_obstacles)  # combine existing obstacles on the map with other robots[for each i: i!=p] in formation
            # follower robot's position correction with local planner
            robots[p + 1].local_planner(obstacles1, params)
            robots_sp[p] = robots[p + 1].sp
