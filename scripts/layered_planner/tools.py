...
import time
import os
from potential_fields import meters2grid
...
def postprocessing(metrics, params, visualize=1):
    ...
    if params.num_robots == 3:
        S0 = 0.5 * sqrt(3) / 2. * params.interrobots_dist ** 2
    elif params.num_robots == 4:
        S0 = sqrt(3) / 2. * params.interrobots_dist ** 2
    else:
        S0 = metrics.area_array[1]
    ...
    if visualize:
        ...
        plt.plot(metrics.t_array, metrics.S_mean * np.ones_like(metrics.t_array), '--', color='r', label='Mean area',
                 linewidth=2)
        plt.xlabel('Time, [s]')
    ...
    if visualize:
        ...
        ax.plot(traj[:, 0], traj[:, 1], 100 * np.ones(traj.shape[0]), label='Centroid trajectory', linewidth=3,
                color='r', zorder=0)
        for robot in metrics.robots:
            traj = meters2grid(robot.route)
            ax.plot(traj[:, 0], traj[:, 1], 100 * np.ones(traj.shape[0]), '--', label="Robots' trajectories",
                    linewidth=1, color='y', zorder=1)
...
def save_data(metrics, folder_name='output_%f' % time.time()):
    ...
    ws.write(0, 0, 'T_reach goal')
    ws.write(0, 1, metrics.t_reach_goal)
    ws.write(1, 0, 'Robots_path_lengths')
    for i in range(len(metrics.robots)):
        ws.write(1, i + 1, metrics.robots[i].path_length)
    ws.write(2, 0, 'Centroid_path_length')
    ws.write(2, 1, metrics.centroid_path_length)
    ...
    ws.write(5, 0, 'S_min')
    ws.write(5, 1, metrics.S_min)
    ws.write(6, 0, 'S_default')
    ws.write(6, 1, metrics.S_default)
    ws.write(7, 0, 'S_mean')
    ws.write(7, 1, metrics.S_mean)
    ws.write(8, 0, 'S_max')
    ws.write(8, 1, metrics.S_max)

    ws.write(9, 0, 'R_formation_mean')
    ws.write(9, 1, metrics.R_formation_mean)

    ws.write(10, 0, 'Vel_mean')
    ws.write(10, 1, metrics.vel_mean)
    ws.write(11, 0, 'Acc_mean')
    ws.write(11, 1, metrics.acc_mean)
    ws.write(12, 0, 'Jerk_mean')
    ws.write(12, 1, metrics.jerk_mean)
    ws.write(13, 0, 'Snap_mean')
    ws.write(13, 1, metrics.snap_mean)

    ws.write(14, 0, 'CPU usage mean')
    ws.write(14, 1, metrics.cpu_usage_mean)
    ws.write(15, 0, 'Memory usage mean')
    ws.write(15, 1, metrics.memory_usage_mean)
