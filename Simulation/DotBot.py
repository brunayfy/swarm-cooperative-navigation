...
class DotBot(object):
    ...
    def __init__(self, dotBotId, floorplan):
        ...
        self.headingRequested = 0  # the heading, a float between 0 and 360 degrees (0 indicates North) as requested
        # by the orchestrator
        self.headingInaccuracy = 0  # inaccuracy, in degrees of the heading. Actual error computed as uniform(-,+)
        ...
        self.speedInaccuracy = 0  # inaccuracy, in m/s of the speed. Actual error computed as uniform(-,+)
    ...
    def getAttitude(self):
        ...
        # update position
        newX = x + (now - posTs) * math.sin(math.radians(headingActual)) * speedActual
        newY = y - (now - posTs) * math.cos(math.radians(headingActual)) * speedActual
    ...
    def _computeNextBump(self):
        ...
        # loop through obstacles, looking for closer bump coordinates
        for obstacle in self.floorplan.obstacles:

            # coordinates of obstacle upper left and lower right corner
            ax = obstacle['x']
            ay = obstacle['y']
            ...
            if (bump_xo is not None) and (bump_tso <= bump_ts):
    ...
    def _computeNextBumpFrame(self):
        ...
        valid_intersections = []
        if north_x is not None and 0 <= north_x <= self.floorplan.width:
            valid_intersections += [(north_x, 0)]
        if south_x is not None and 0 <= south_x <= self.floorplan.width:
            valid_intersections += [(south_x, self.floorplan.height)]
        if west_y is not None and 0 <= west_y <= self.floorplan.height:
            valid_intersections += [(0, west_y)]
        if east_y is not None and 0 <= east_y <= self.floorplan.height:
            valid_intersections += [(self.floorplan.width, east_y)]

        # if more than 2 valid points, pick the pair that is the furthest apart
    ...
    def _computeNextBumpObstacle(self, rx, ry, x2, y2, ax, ay, bx, by):
        ...
        Function implements the Liang-Barsky algorithm
        ...
        # if p = 0 and q<0 then line completely outside boundaries
